...
class GUI(Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        frame = Variables(container, self)
        
        self.frames[Variables] = frame
        
        frame.grid(row=0, column=0, sticky="nsew")
        
        self.show_frame(Variables)
...
class Variables(Frame):
    ...
    def set_entries(self):
        ...
        for x in range(0, len(self.texts)):
            #create entry 
            self.entries[self.texts[x]] = Entry(self, textvariable=self.text_variable[self.texts[x]])
    ...
    def save_option(self): 
        ...
        #create button
        buttonSave = Button(self,text="Save (exclude .json)",command=lambda: self.save()).grid(row=0,column=1)
    
    #creates label, entry, and button for uploading all values    
    def upload_option(self):   
        
        #create button
        buttonUpload = Button(self,text="Upload",command=lambda: self.upload()).grid(row=0,column=0)
    ...
    def tab_buttons(self):
        ...
        buttonAll = Button(self,text="All",command=lambda: self.use_all())
        buttonAll.grid(row=1,column=2)
        #button to display commonly used variables
        buttonCommon = Button(self,text="Common",command=lambda: self.use_common())
        buttonCommon.grid(row=2,column=2)
        #button to display part parameters
        buttonParts = Button(self,text="Parts",command=lambda: self.use_parts())
        buttonParts.grid(row=3,column=2)
        #button to display layer parameters
        buttonLayers = Button(self,text="Layers",command=lambda: self.use_layers())
        buttonLayers.grid(row=4,column=2)
        #button to display file parameters
        buttonFiles = Button(self,text="Files",command=lambda: self.use_files()).grid(row=5,column=2)
    ...
    def presets(self):
        ...
        #button for dogbone
        buttonDogbone = Button(self,text="Dogbone",command=lambda: self.dogbone())
        buttonDogbone.grid(row=1,column=3)
    
    #create button to convert to Gcode
    def gcode(self):
        
        buttonGcode = Button(self,text="Convert to Gcode",command=lambda: self.convert())
